...
class CustomAppBar extends StatefulWidget implements PreferredSizeWidget {
  ...
  CustomAppBar({super.key, this.height = 88,required this.scaffoldKey});
...
class _CustomAppBarState extends State<CustomAppBar> {
                    ...
                    if (!isMobile)
                      Expanded(
                        child: Text(
                          LocalStrings.websiteName,
                          style: isTablet
                              ? AppConstants.nunitoMediumPrimaryColorW700
                              : AppConstants.nunitoBigPrimaryColorW700,
                        ),
                      ),
                    if (isLargeScreen) ...[
                      Spacer(),
  ...
  // Navigation Item with Active Tab Highlighting
    ...
    return Padding(
      ...
      child: TextButton(
        child: Text(
          text,
          style: isActive
              ? AppConstants.nunitoMediumPrimaryColorW700.copyWith(
            fontWeight: FontWeight.bold
          ) // Highlight active tab
              : AppConstants.nunitoMediumW500,
        ),
        onPressed: () {
          if (!isActive) {
            context.go(route);
